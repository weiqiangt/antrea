...
func TestConnectivityFlows(t *testing.T) {
	...
	c = ofClient.NewClient(br, bridgeMgmtAddr, net.ParseIP("169.254.169.110"), nil, true, false)
...
func TestIPv6ConnectivityFlows(t *testing.T) {
	...
	c = ofClient.NewClient(br, bridgeMgmtAddr, nil, nil, true, false)
	err := ofTestUtils.PrepareOVSBridge(br)
	require.Nil(t, err, fmt.Sprintf("Failed to prepare OVS bridge: %v", err))
...
func testInstallGatewayFlows(t *testing.T, config *testConfig) {
	gatewayConfig := config.nodeConfig.GatewayConfig
	err := c.InstallGatewayFlows(config.nodeConfig.NodeIPAddr.IP)
	if err != nil {
