...
// runAntctl runs antctl commands on a antrea pods, controller or agent. It always
// runs the commands with verbose enabled.
...
// TestAntctlAgentLocalAccess ensures antctl is accessible in controller pod.
...
// the kubernetes cluster. It uses the antctl binary copied from the controller pod.
func TestAntctlControllerRemoteAccess(t *testing.T) {
	...
	// Make the antctl binary executable on the master node.
...
// TestAntctlVerboseMode ensures no unexpected outputs in stderr and stdout.
