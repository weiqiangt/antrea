...
func TestAntctlAgentFlowTable(t *testing.T) {
	...
	if _, _, err := runAntctl(podName, []string{"antctl", "-v", "get", "flow-table"}, data, t); err != nil {
		t.Fatalf("Error when running `antctl get flow-table` from %s: %v", podName, err)
	}
	if _, _, err := runAntctl(podName, []string{"antctl", "-v", "get", "flow-table", "20"}, data, t); err != nil {
