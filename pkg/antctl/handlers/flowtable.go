...
func (s *FlowTable) Handler(aq monitor.AgentQuerier) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var flowTables []flowtable.Response
		...
		for tableID, flowCount := range aq.GetOVSFlowTable() {
			resp := flowtable.Response{TableID: tableID, FlowCount: uint(flowCount)}
