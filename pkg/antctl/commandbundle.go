...
// commandList organizes CommandDefinitions.
...
func (cl *commandList) ApplyToMux(mux *mux.PathRecorderMux, aq monitor.AgentQuerier, cq monitor.ControllerQuerier) {
	for _, cmdOpt := range cl.CommandDefinitions {
...
func (cl *commandList) ApplyToRootCommand(root *cobra.Command, isAgent bool, inPod bool) {
	...
	for i := range cl.CommandDefinitions {
		cmdOpt := cl.CommandDefinitions[i]
	...
	root.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		...
		root.Flags().Set("logtostderr", fmt.Sprint(enableVerbose))
		root.Flags().Set("alsologtostderr", fmt.Sprint(enableVerbose))
		if enableVerbose {
			root.Flags().Set("v", "0")
		}
	}
...
func (cl *commandList) validate() []error {
	var errs []error
	if len(cl.CommandDefinitions) == 0 { // must has at least one applySubCommandToRoot
		return []error{fmt.Errorf("no applySubCommandToRoot in applySubCommandToRoot bundle")}
	}
	for i, c := range cl.CommandDefinitions { // each commandDefinition must be valid
