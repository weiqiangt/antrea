...
func TestCommandBundleApplyToCommand(t *testing.T) {
	...
	TestBundle.ApplyToRootCommand(testRoot, true, false)
	// sub-applySubCommandToRoot should be attached
...
// TestParseBundle ensures the commandBundle could be correctly parsed.
func TestParseBundle(t *testing.T) {
	r := mux.NewPathRecorderMux("")
	assert.Len(t, TestBundle.validate(), 0)
	TestBundle.ApplyToMux(r, nil, nil)
