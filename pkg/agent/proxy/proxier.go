...
func (p *proxier) installServices() {
	for svcPortName, svcPort := range p.serviceMap {
		...
		if needRemoval && svcInfo.NodePort() > 0 {
			if err := p.ofClient.UninstallServiceFlows(p.virtualNodePortIP, uint16(pSvcInfo.NodePort()), pSvcInfo.OFProtocol); err != nil {
				klog.Errorf("Error when removing NodePort Service flows: %v", err)
				continue
			}
			if err := p.routeClient.DeleteNodePort(p.nodeIPs, pSvcInfo, p.isIPv6); err != nil {
		...
		if svcInfo.NodePort() > 0 {
			if err := p.ofClient.InstallServiceFlows(groupID, p.virtualNodePortIP, uint16(svcInfo.NodePort()), svcInfo.OFProtocol, uint16(svcInfo.StickyMaxAgeSeconds())); err != nil {
				klog.Errorf("Error when installing Service NodePort flow: %w", err)
				continue
			}
			if err := p.routeClient.AddNodePort(p.nodeIPs, svcInfo, p.isIPv6); err != nil {
				klog.Errorf("Error when installing Service NodePort rules: %w", err)
				continue
			}
		}
...
func NewDualStackProxier(
	...
	ipv4Proxier, err := NewProxier(virtualNodePortIP, nodePortAddresses, hostname, podIPv4CIDR, informerFactory, ofClient, routeClient, false)
	if err != nil {
		return nil, err
	}

	// Create an ipv6 instance of the single-stack proxier.
	ipv6Proxier, err := NewProxier(virtualNodePortIPv6, nodePortAddresses, hostname, podIPv6CIDR, informerFactory, ofClient, routeClient, true)
	if err != nil {
		return nil, err
	}
