...
func (c *client) Initialize() error {
	...
	for _, flow := range c.defaultFlows() {
		flow = flow.SetCookie(c.cookieAllocator.Request(cookie.Default).Raw())
		if err := c.flowOperations.Add(flow); err != nil {
			return fmt.Errorf("failed to install default flows: %v", err)
		}
	}
	if err := c.flowOperations.Add(c.arpNormalFlow().SetCookie(c.cookieAllocator.Request(cookie.Default).Raw())); err != nil {
		return fmt.Errorf("failed to install arp normal flow: %v", err)
	}
	if err := c.flowOperations.Add(c.l2ForwardOutputFlow().SetCookie(c.cookieAllocator.Request(cookie.Default).Raw())); err != nil {
