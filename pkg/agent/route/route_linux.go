...
func NewClient(nodePortVirtualIP net.IP, nodePortVirtualIPv6 net.IP, serviceCIDR *net.IPNet, networkConfig *config.NetworkConfig, noSNAT bool) (*Client, error) {
	return &Client{
...
func (c *Client) restoreNodePortV4IptablesDataV4(hostGateway string) *bytes.Buffer {
...
func generateNodePortIPSETEntries(nodeIP net.IP, isIPv6 bool, svcInfos []*types.ServiceInfo) sets.String {
...
func (c *Client) Reconcile(podCIDRs []string) error {
	...
	for i := range routes {
		...
		if features.DefaultFeatureGate.Enabled(features.AntreaProxy) && route.Dst != nil && (route.Dst.Contains(c.nodePortVirtualIP) || route.Dst.Contains(c.nodePortVirtualIPv6)) {
			continue
		}
	...
	for neighIP, actualNeigh := range actualNeighbors {
		...
		if c.nodePortVirtualIPv6 != nil && c.nodePortVirtualIPv6.String() == neighIP {
			continue
		}
...
func (c *Client) ReconcileNodePort(nodeIPs []net.IP, svcEntries []*types.ServiceInfo) error {
	...
	reconcile := func(setName string, isIPv6 bool, desiredSvcEntries []*types.ServiceInfo) error {
