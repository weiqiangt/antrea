...
const (
	...
	// TODO: persist store the ID just like the round ID of the openflow client.
	antreaRouteTableID = 110
	...
	antreaClusterIPSet  = "ANTREA-SVC-CLUSTER-SERVICE"
	antreaClusterIPSet6 = "ANTREA-SVC-CLUSTER-SERVICE6"
	...
	antreaClusterServicesChain  = "ANTREA-SVC-CLUSTER"
...
func (c *Client) syncIPSet() error {
	if c.nodeportSupport {
		...
		if err := ipset.CreateIPSet(antreaClusterIPSet, ipset.HashIPPort, false); err != nil {
			return err
		}
		if err := ipset.CreateIPSet(antreaClusterIPSet6, ipset.HashIPPort, true); err != nil {
			return err
		}
	}
...
func (c *Client) syncIPTables() error {
	...
	jumpRules := []struct {
		...
		{c.nodeportSupport, iptables.NATTable, iptables.PostRoutingChain, antreaServicesMasqChain, "Antrea: jump to Antrea NodePort Service masquerade rules", true},
		{c.nodeportSupport, iptables.MangleTable, iptables.PreRoutingChain, antreaClusterServicesChain, "Antrea: jump to Antrea Cluster Service rules", true},
		{c.nodeportSupport, iptables.MangleTable, iptables.OutputChain, antreaClusterServicesChain, "Antrea: jump to Antrea Cluster Service rules", true},
		{c.nodeportSupport, iptables.NATTable, iptables.PreRoutingChain, antreaClusterServicesChain, "Antrea: jump to Antrea Cluster Service rules", true},
		{c.nodeportSupport, iptables.NATTable, iptables.OutputChain, antreaClusterServicesChain, "Antrea: jump to Antrea Cluster Service rules", true},
	}
...
func (c *Client) restoreClusterServiceNatIptablesData(hostGateway string, isIPv6 bool) *bytes.Buffer {
	iptablesData := new(bytes.Buffer)
	writeLine(iptablesData, []string{
		"-A", antreaClusterServicesChain,
		"-m", "mark", "--mark", clusterIPServiceMark,
...
func (c *Client) restoreClusterServiceMangleIptablesData(hostGateway string, isIPv6 bool) *bytes.Buffer {
	set := antreaClusterIPSet
	if isIPv6 {
		set = antreaClusterIPSet6
	}
	iptablesData := new(bytes.Buffer)
	writeLine(iptablesData, []string{
		"-A", antreaClusterServicesChain,
		"-m", "mark", "--mark", clusterIPServiceMark,
		"-j", iptables.MarkTarget, "--set-mark", "0",
	}...)
	...
	writeLine(iptablesData, []string{
		"-A", antreaClusterServicesChain,
		"-m", "set", "--match-set", set, "dst,dst",
		"-j", iptables.MarkTarget, "--set-mark", clusterIPServiceMark,
	}...)
	return iptablesData
}
...
func (c *Client) restoreIptablesData(podCIDR *net.IPNet, podIPSet string) *bytes.Buffer {
	...
	// TODO: should not be NodePort here.
	// TODO: should not be NodePort here.
	// TODO: should not be NodePort here.
	// TODO: should not be NodePort here.
	if c.nodeportSupport {
		writeLine(iptablesData, iptables.MakeChainLine(antreaClusterServicesChain))
		io.Copy(iptablesData, c.restoreClusterServiceMangleIptablesData(hostGateway, isIPv6))
	}
	writeLine(iptablesData, "COMMIT")
	...
	if c.nodeportSupport {
		...
		io.Copy(iptablesData, c.restoreClusterServiceNatIptablesData(hostGateway, isIPv6))
	}
	writeLine(iptablesData, "COMMIT")
	return iptablesData
}
...
func (c *Client) initIPRoutes() error {
	...
	if c.nodeportSupport {
		if err := c.initServiceRouteTable(gwLink, c.nodeConfig.NodeIPAddr.IP); err != nil {
			return err
		}
	}
	return nil
...
func (c *Client) initServiceRouteTable(gwLink netlink.Link, gwIP net.IP) error {
	// TODO: polish here
	serviceHostVirtualIP := net.ParseIP("169.254.169.111").To4()
	route := &netlink.Route{
		LinkIndex: gwLink.Attrs().Index,
		Gw:        serviceHostVirtualIP,
		Table:     antreaRouteTableID,
		Flags:     int(netlink.FLAG_ONLINK),
	}
	if err := netlink.RouteReplace(route); err != nil {
		return fmt.Errorf("failed to add default service route to table: %w", err)
	}
	...
	for _, existRule := range rules {
		if existRule.Mark == serviceFwMark &&
			existRule.Table == antreaRouteTableID {
			klog.Infoln("Service route rule exist, skip installing")
			return nil
		}
	}
	...
	rule.Priority = 10
	rule.Mark = serviceFwMark
	rule.Table = antreaRouteTableID
	if err := netlink.RuleAdd(rule); err != nil {
		return fmt.Errorf("failed to add ip rule for forwarding ClusterIP traffic: %w", err)
	}
...
func (c *Client) Reconcile(podCIDRs []string) error {
	...
	for i := range routes {
		...
		if route.Dst != nil && (route.Dst.Contains(net.ParseIP("169.254.169.111"))) {
			continue
		}
...
func (c *Client) addHack() error {
	var route *netlink.Route
	nodeportIP := net.ParseIP("169.254.169.111")
	route = &netlink.Route{
		Dst: &net.IPNet{
			IP:   nodeportIP,
			Mask: net.IPv4Mask(255, 255, 255, 255),
...
func (c *Client) AddNodePortService(nodeIPs []net.IP, svcInfo *proxytypes.ServiceInfo, isIPv6 bool) error {
...
func (c *Client) AddService(svcIP net.IP, svcPort uint16, protocol binding.Protocol) error {
	setName := antreaClusterIPSet
	if svcIP.To4() == nil {
		setName = antreaClusterIPSet6
	}
	if err := ipset.AddEntry(setName, fmt.Sprintf("%s,%s:%d", svcIP.String(), strings.ToLower(string(protocol)), svcPort)); err != nil {
		klog.Errorf("Error when adding ClusterIP Service to ipset %s: %v", setName, err)
	}
	return nil
}

func (c *Client) DeleteService(svcIP net.IP, svcPort uint16, protocol binding.Protocol) error {
	setName := antreaClusterIPSet
	if svcIP.To4() == nil {
		setName = antreaClusterIPSet6
	}
	if err := ipset.DelEntry(setName, fmt.Sprintf("%s,%s:%d", svcIP.String(), strings.ToLower(string(protocol)), svcPort)); err != nil {
		klog.Errorf("Error when deleting ClusterIP Service from ipset %s: %v", setName, err)
	}
	return nil
}

func (c *Client) DeleteNodePortService(nodeIPs []net.IP, svcInfo *proxytypes.ServiceInfo, isIPv6 bool) error {
	setName := antreaNodePortClusterSet
	if isIPv6 {
		setName = antreaNodePortClusterSet6
	}
