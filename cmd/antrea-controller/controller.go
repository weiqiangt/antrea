...
func run(o *Options) error {
	...
	if err := antctl.Definition.ApplyToAPIServer(readyAPIServer.GenericAPIServer, controllerMonitor); err != nil {
		return fmt.Errorf("error applying antctl to API server: %v", err)
	}
	go readyAPIServer.Run(stopCh)

	// Start the local server
	antctlServerOpt, err := antctl.NewServerOptions(antctl.Definition, nil, controllerMonitor, antctl.DefaultControllerPort)
	if err != nil {
