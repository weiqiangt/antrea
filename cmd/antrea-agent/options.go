...
func (o *Options) validateAntreaProxyConfig() error {
	if features.DefaultFeatureGate.Enabled(features.AntreaProxy) {
		...
		if o.config.NodePortVirtualIPv6 != "" { // It must be a unique local address.
			_, linkLocalNet, _ := net.ParseCIDR("fec0::/10")
			if !linkLocalNet.Contains(net.ParseIP(o.config.NodePortVirtualIPv6)) {
				return fmt.Errorf("NodePortVirtualIPv6 %s is not an valid link-local IP address", o.config.NodePortVirtualIPv6)
			}
		}
